...
The checkpoint is an exception path, not a standard phase of every review cycle.
...
Reviewer wording is not a source of truth. Finding validation maps each material
candidate to a canonical selected `risk_surface_id` or records it as
...
- classification is disputed, such as `contract_gap` versus
  `implementation_defect`;
- repeated P1 findings come from different reviewers and root cause is unclear;
...
substantive review gate.

For the MVP this may be a structured section in the next review-cycle or
review-fix-loop report. A standalone closure schema should be added only if
later validation needs justify it.
