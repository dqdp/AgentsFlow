...
- `big-feature-contract-first` — v0.2 supported target workflow. Define a behavioral contract, plan, impact map, gates, and evidence before implementing a large feature.
- `new-project-spec-first` — reference/next in v0.2. Turn a vague project idea into specifications, initial contracts, ADR candidates, and roadmap before implementation.
- `bugfix-regression-capture` — reference/next in v0.2. Reproduce a bug, capture a regression scenario, implement a minimal fix, and verify the regression does not return.
...
- `review-only-fusion` — v0.2 review utility workflow, not a supported `prepare-workflow.target_workflow`.
