# AgentsFlow Checkpoint — v0.1.4

Date: 2026-06-17
Status: design checkpoint after review-agent relevance-validation rule
...
### 6. Review findings require relevance validation

Review-agent findings are **candidate findings**, not authoritative truth.

They become accepted issues only after the main/orchestrating agent validates their relevance against:

- task contract;
- artifact or diff;
- verification gate report;
- evidence bundle and logs;
- relevant ADRs and accepted decisions;
- workflow profile, non-goals, and scope.

Possible relevance statuses:
...
P0/P1 candidate findings must not be silently discarded. If the main/orchestrating agent rejects or downgrades a plausible blocker, it must record the reason and supporting evidence.

Fusion reports are decision support, not final truth.

### 7. BDD / contract layer
...
- planning agents and review agents are both read-only but have different roles and timing;
- the main/orchestrating agent validates reviewer finding relevance before acting on findings.
...
Current v0.1.4 uses `integrations/haft/` as the clearer boundary.
...
### G. BDD scenario binding

Need to decide how BDD scenarios bind to actual checks:
...
### H. Multi-model fusion policy details

Need to define:

- model diversity policy;
- independent vs sequential reviewer semantics;
- disagreement handling;
- relevance validation handoff from fusion to main/orchestrating agent.
